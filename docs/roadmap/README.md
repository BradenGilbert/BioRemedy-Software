...
| 06 | [Opportunity Domain Correctness](phase-06-opportunity-domain.md) | 🟢 **Substantial progress 2026-09-17** — fixed the create-opportunity navigation bug, consolidated two parallel/non-synced "associated contact" systems onto the one that actually worked (fixing the wrong-edit-dialog bug, the "editing a contact drops other opportunities" data-loss bug, and adding a proper add/remove/"Formerly Associated" UI in one pass), added Open Project button, fixed a floating-checkbox CSS bug, removed boxy button styling from two badge lists. Remaining items need owner decisions (close date, Opportunity Type vocabulary, "Regarding" field) or are standalone builds (multi-contact activities, site-walk scheduling, advance-button validation highlighting) |
