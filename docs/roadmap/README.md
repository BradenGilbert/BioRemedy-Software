...
The felt pain. Almost every item on the 2026-09-15 and 2026-09-16 feedback lists lives here. These are data-model problems wearing UI clothes, which is why they read as "doesn't make sense" rather than as bugs. Phases 06–09 were added 2026-09-16 from a second feedback pass (`bioremedy crm notes 9.16.2026.docx`) that went deep on Opportunities, Dispatch/Operations, and two brand-new modules the owner called out explicitly as needed builds (Quotes/Estimates, Billing/Invoicing). Phases 10–11 (Front Line, Field Ops Depth) were reprioritized into this stage on 2026-09-17, per an explicit owner decision to work them right after Billing & Invoicing rather than post-pilot — this intentionally pushes the Pilot Milestone later, since real field-app depth now lands before the identity/documents gate rather than after it.
...
| 08 | [Quotes & Estimates](phase-08-quotes-estimates.md) | ✅ **Shipped 2026-09-17, live bug follow-up shipped 2026-09-17 (2nd session)** — real line-item builder for both Quote and Estimate (two separate documents, owner decision), new Rate Card admin screen (products/units/price levels) now the single source of truth Phase 04's vendor-compliance and service-agreement panels already read, project value now defaults from the linked quote/estimate total when creating a project from a Won opportunity; follow-up added an explicit "Mark current" override (previously fully automatic), print/export to a customer-presentable view, notes, billing/shipping address (linked to the existing `addresses` collection), and optional/write-in line items excluded from the total |
| 09 | [Billing & Invoicing](phase-09-billing-invoicing.md) | ✅ **Shipped 2026-09-17** — real "close project" action gated on the Closeout stage, auto-generated cost report/P&L snapshot from real equipment/labor/material usage priced from Phase 08's rate card, "Regenerate" action for post-close usage, feeds (doesn't replace) the existing invoice dialog |
| 10 | [Front Line — Real Tiles](phase-10-frontline.md) | ✅ **Both parts shipped 2026-09-17** — owner decision: build the web-hosted simulator's tiles to full feature depth (mobile app / production auth / offline sync decisions explicitly deferred). Part 1 (Time Sheet, Trips, Settings) and Part 2 (Forms, Messaging, Location, Invoices, plus the concrete-gaps backlog: repeatable Odometer/Sample capture, sample photo fields, "+ Activity" ad hoc capture, PPE negative-quantity confirm-and-flag, blocked in-place reschedule of completed jobs, and job-status-progression verification) are real and verified live. Waste-tracking permit capture stays deferred to Phase 11/13 per the phase doc. |
| 11 | [Field Ops Depth & Reporting](phase-11-field-ops-depth.md) | 🟡 Outline, fleshed out 2026-09-16 with concrete field-app gaps — not yet a session-ready plan. Reprioritized ahead of the Pilot Milestone 2026-09-17 (owner decision). |
...
| 13 | [Document Storage](phase-13-documents.md) | Not started |

> ### ◆ PILOT MILESTONE
> After Phase 13, real users can be put on real data safely.

### Stage D — Production Database

| Phase | Name | Status |
|---|---|---|
| 14 | [PostgreSQL Migration](phase-14-postgres-migration.md) | Not started |
...
| Permit registry + per-job waste tracking | 11 | TCEQ/EPA-driven — BioRemedy's 10-day storage permit, oily waste handler permit, and others need to be tracked and tied to job cost. |
| Identity/RBAC/audit tables | 12 | Listed as Priority 0 in `docs/database-handoff-map.md`; still absent. |
| Generic entity-attachment store | 13 | Every Files tab is an empty stub. |
...
- **Phase 09 — Billing & Invoicing** — a real "close project" action (`closeProject()`), gated on `projects.projectStage` having reached the existing `PROJECT_STAGES` ladder's terminal "Closeout" stage (built on top of Phase 07 item 11's fix, not a new parallel status); closing generates a stored cost-report/P&L snapshot (`projects.closeReport`) from real `materialUsage`/`equipmentLogs` and a new `laborHoursForProject()` (summing Front Line Timer-task hours across the project's dispatch jobs — the one real per-job hours source, since Phase 07 item 12's `employees.hoursWorked` fix is cross-project cumulative, not per-job), priced via Phase 08's rate card; found and filled a real gap where that rate card had no per-hour/per-day/per-unit cost basis (only whole-engagement day-rate services), by adding three generic cost-basis products (`prod-field-labor-standard`, `prod-equipment-standard-daily`, `prod-material-standard-unit`) to the seed data. Owner decisions: closing does not lock out further usage entries — a "Regenerate" action re-runs the generator and overwrites the snapshot in place (no history kept, by design, pending Phase 12's real audit log); the report is a snapshot generated at close/regenerate time only, never recomputed on view; waste-disposal cost (Phase 11) explicitly does not feed this P&L yet. `getFinanceRows()`/`openInvoiceDialog()` now prefer the generated report's numbers once one exists, which is how it feeds the existing manual invoice dialog rather than bypassing it. Verified live via Playwright: closed a real project with real usage data, confirmed report numbers and invoice-dialog prefill, confirmed post-close usage doesn't change the report until an explicit regenerate, and confirmed a zero-usage/no-quote project still closes cleanly to an all-zero report with no console errors.
...
| Item | Slotted into |
|---|---|
| ~~`projects.projectStage` (formerly `jobs.projectStage`) is written at creation but never read~~ | ✅ Fixed 2026-09-17 in Phase 07 (item 11): `getJobProgress()` now reads `projectStage` when present, and dispatch-status transitions write it forward via `advanceProjectStageFromDispatchStatus`. The related Phase 10 note ("job status progression does not move along with work plan completion") may still need its own check once Phase 10 work starts — this fix covers the dispatch-job-status → project-stage path, not work-plan-completion → project-stage |
...
| Attachment inline-view route is not role-gated (relies on opaque IDs) | Phase 12 |
| QuickBooks integration is a mock export queue | Phase 11 |
...
- `bioremedy crm notes 9.16.2026.docx` (project root) — second feedback pass, 2026-09-16. ~70 items, mostly Opportunity/Contact/Dispatch bugs plus two explicitly-flagged new modules (Quotes/Estimates, Billing/Invoicing). Fed Phases 03–09, 10, 11, 13 in this revision. Items were verified against running code before being written into phase docs where feasible — see each phase's own citations; Phase 07 and most of Phase 10/11's additions are **not yet verified**, only transcribed, and say so explicitly in their own docs.
